...
contract Utils is Test {
  ...
  uint constant alicePk = 0xa11ce;
  uint constant bobPk = 0xb0b;
  uint constant liquidatorPk = 0x110;
  ...
  function prepareFund(
    ...
    // approve tokens
    IERC20(_tokenA).safeApprove(_integration, type(uint).max);
    IERC20(_tokenB).safeApprove(_integration, type(uint).max);
    IERC20(_lp).safeApprove(_integration, type(uint).max);
  ...
  function prepareFundV2(
    ...
    for (uint i = 0; i < _tokens.length; i++) {
      IERC20(_tokens[i]).safeApprove(_integration, type(uint).max);
    }
    IERC20(_lp).safeApprove(_integration, type(uint).max);

    // mint tokens
    for (uint i = 0; i < _tokens.length; i++) {
      deal(_tokens[i], _user, 1_000 * 10**IERC20Metadata(_tokens[i]).decimals());
    }
    deal(_lp, _user, 1000);
